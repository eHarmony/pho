...
public class HBaseRedisFeedMergeStrategyTest {
	...
	public void testMerge_SameTime() throws Exception{
		...
		merger.mergeMatchByTimestamp(matchId, oneMatch, 
							"lastModifiedDate", oneMatch, "lastModifiedDate");
	...
	public void testMerge_TargetIsNewer() throws Exception{
		...
		target.get(MatchFeedModel.SECTIONS.MATCH).put(MatchFeedModel.MATCH.LAST_MODIFIED_DATE, System.currentTimeMillis());
		...
		merger.mergeMatchByTimestamp(matchId, target, 
	...
	public void testMerge_DeltaIsNewer() throws Exception{
		...
		Map<String, Map<String, Object>> target = match.getMatches().get(matchId);
		Map<String, Map<String, Object>> delta = match2.getMatches().get(matchId);
		delta.get(MatchFeedModel.SECTIONS.MATCH).put(MatchFeedModel.MATCH.LAST_MODIFIED_DATE, System.currentTimeMillis());
